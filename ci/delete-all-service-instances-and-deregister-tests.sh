...
pushd $(dirname $0)/..
  ...
  pushd src/github.com/pivotal-cf/on-demand-service-broker
    ginkgo -randomizeSuites=true -randomizeAllSpecs=true -keepGoing=true -race -failOnPending system_tests/delete_all_service_instances_and_deregister_broker_tests
  popd
popd
